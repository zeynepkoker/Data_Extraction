...
for pres in pressure:
	os.chdir("Pressure_" + pres + "_Volume_Surface_Changes_Measurement")
	if column_value == 9:
		X = []
		Y = []
		charge_value = []
		Title_name = "Volume Changes"
		mean_file = open( str(Title_name) +"_mean.txt", "w")
		for file_name in glob.glob("*_Volume_changes.dat"):
			charge_value.append(file_name.replace('_Volume_changes.dat', '').replace('2_1_','Charge: ').replace('_3_6','').replace('_','.').replace(pres,''))
			x = []
			y = []
			...
			file_name_splitted = file_name_splitted.replace('_Volume_changes.dat', '\t')
	...
	if column_value == 11:
		...
		Y = []
		charge_value = []
		Title_name = "Surface Volume Changes"
		mean_file = open( str(Title_name) +"_mean.txt", "w")
		for file_name in glob.glob("*_Volume_changes.dat"):
			charge_value.append(file_name.replace('_Volume_changes.dat', '').replace('2_1_','Charge: ').replace('_3_6','').replace('_','.').replace(pres,''))
			x = []
